...
async def upload_file(file, folder: str, user_id: str, invoice_no: str = None):
    ...
    try:
        ...
        # The specific 409 error check is now less critical but can be kept as a fallback.
        if "The resource already exists" in str(e) or "Duplicate" in str(e):
             raise HTTPException(status_code=409, detail="File upload failed due to a conflict. Please try again.")
        raise e
