...
if not api_key:
    raise ValueError("CRITICAL: GEMINI_API_KEY is missing from environment variables. Please check your .env file.")
...
async def embed_and_store_invoice(invoice_id: int, full_data: Dict[str, Any]):
    """
    Stores an invoice as multiple vectors:
    1. A summary vector (Buyer, Date, Total)
    2. Item vectors (Specific Product details)
    """
    try:
        supabase = await get_supabase_admin_client()
        
        # Extract Data
        inv = full_data['invoice']
        buyer = full_data['buyer']
        items = full_data['items']
        user_id = full_data.get('user_id') 

        if not user_id:
            logger.error("Embedding failed: user_id missing in payload")
            return
        ...
        # --- 2. CREATE SUMMARY CHUNK ---
        summary_text = (
            f"Invoice #{inv['number']} Type: {inv['title']}. "
            f"Date: {inv['date']}. "
            f"Buyer: {buyer['name']}, GSTIN: {buyer.get('gstin', 'N/A')}. "
            f"Address: {buyer.get('address', 'N/A')}. "
        )
        
        summary_vector = await embeddings.aembed_query(summary_text)
        ...
        # --- 3. CREATE ITEM CHUNKS ---
        for item in items:
            item_text = (
                f"Line Item in Invoice #{inv['number']} for {buyer['name']}: "
                f"Product: {item['name']}. "
                f"Description: {item.get('description', '')}. "
                f"Quantity: {item['quantity']} {item.get('unit', 'pcs')}. "
                f"Rate: {item['rate']}. "
                f"HSN: {item.get('hsn', 'N/A')}."
            )
            
            item_vector = await embeddings.aembed_query(item_text)
        ...
        if vectors_to_insert:
            await supabase.table('invoice_embeddings').insert(vectors_to_insert).execute()
            logger.info(f"✅ Stored {len(vectors_to_insert)} vectors for Invoice {inv['number']}")

    except Exception as e:
        logger.error(f"Embedding Error: {e}", exc_info=True)

async def semantic_search_invoices(user_id: str, query: str):
    try:
        supabase = await get_supabase_admin_client()
        ...
        # 2. Search DB using the RPC function created earlier
        response = await supabase.rpc("match_invoice_details", {
        ...
        if not response.data:
            return "No relevant invoice details found."
        ...
        return "Error searching invoice history."