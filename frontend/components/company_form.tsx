...
// --- Icons ---
import { Building, Landmark, Image as ImageIcon, UploadCloud, Mail, Info, ExternalLink, XCircle, Save, ArrowRight } from "lucide-react";
...
const uploadAsset = async (
    ...
    if (existingUrl) {
        try {
            ...
            console.warn(`Could not delete old asset.`, e);
        }
    }
    const newFilePath = `${userId}/${storagePath}/${Date.now()}_${file.name}`;
    const { error: uploadError } = await supabase.storage.from('company-assets').upload(newFilePath, file);
...
export default function CompanyInfoForm() {
    ...
    // --- ✅ FIX #1: Update function to accept event and prevent default ---
    const handleNextTab = (e: React.MouseEvent<HTMLButtonElement>) => {
        e.preventDefault(); // This is the key change to stop the form from submitting
        if (!isLastSection) {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                <div className="lg:col-span-3">
                    <form onSubmit={handleSubmit}>
                        <Card className="shadow-lg rounded-xl">
                            <CardContent className="p-6 sm:p-8 space-y-8">
                                {/* ... Your tab content remains the same ... */}
                                {activeSection === 'company' && (
                                    <section className="space-y-6">
                                        <CardTitle>Company Details</CardTitle>
                                        <div className="space-y-2">
                            ...
                            <div className="p-6 sm:p-8 border-t flex justify-end">
                                {isLastSection ? (
                                    ...
                                    // --- ✅ FIX #2: Pass the event to the handler ---
