...
//     return (
//         <DashboardLayout>
//             {/* KEY CHANGE: The top-level content div needs to be responsive.
//               p-3 sm:p-4 md:p-6 is good, but ensuring the overall layout doesn't create overflow.
//               The issue is often in the DashboardLayout, but we can't change it.
//               The use of 'container mx-auto' is removed from here to rely on the DashboardLayout's structure
//               and instead applying responsive padding for small screens to prevent overflow.
//             */}
//             <div className="p-3 sm:p-4 md:p-6 space-y-6 pb-24 **w-full min-w-0**">
//                 <PageHeader />
//                 {/* Ensure the main grid is responsive */}
//                 <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
//                     <div className="lg:col-span-2 space-y-6">
//                         <FormSectionCard title="Client Details" icon={<User className="h-5 w-5"/>}>
//                             <div className="space-y-4">
//                                 {/* Using grid-cols-1 on small screens is good */}
//                                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
//                                     <div className="space-y-1.5 relative" ref={clientDetailsRef}>
//                                         <Label htmlFor="client_name">Client Name</Label>
//                                         <Input id="client_name" value={formState.client_name} onChange={(e) => handleInputChange('client_name', e.target.value)} onFocus={() => setIsClientListVisible(true)} required autoComplete="off" />
...
//                                             )}
//                                         </div>
//                                         {/* Updated grid for better stacking on small screens, if needed */}
//                                         <div className="grid grid-cols-3 gap-3">
//                                             <div className="space-y-1.5"><Label htmlFor={`hsn_${i}`}>HSN</Label><Input id={`hsn_${i}`} value={p.hsn} onChange={(e) => handleProductChange(i, "hsn", e.target.value)} /></div>
//                                             <div className="space-y-1.5"><Label htmlFor={`qty_${i}`}>Qty</Label><Input id={`qty_${i}`} type="number" value={p.quantity} onChange={(e) => handleProductChange(i, "quantity", e.target.value)} /></div>
//                                             <div className="space-y-1.5"><Label htmlFor={`unit_${i}`}>Unit</Label><Input id={`unit_${i}`} value={p.unit} onChange={(e) => handleProductChange(i, "unit", e.target.value)} /></div>
...
//                 </div>
//             </div>
//             {/* This footer section is correctly using container mx-auto and flexible padding (px-4 md:px-6) 
//               which should not cause overflow itself.
//             */}
//             <div className="sticky bottom-0 left-0 right-0 z-10 bg-background/95 backdrop-blur-sm border-t">
//                 <div className="container mx-auto flex items-center justify-end h-16 px-4 md:px-6 gap-4">
//                     <Button variant="outline" onClick={() => setIsTemplateDialogOpen(true)}>
...
//                         {isSaving ? <SvgLoader /> : <><Save className="mr-2 h-4 w-4" /> Create & Download</>}
//                     </Button>
//                 </div>
//             </div>
//             <TemplateDialog 
