...
interface Seller {
    id: string; name?: string; address?: string; state?: string; gstin?: string; contact?: string | null; email?: string | null; logo_url?: string; bank_name?: string; account_no?: string; ifsc_code?: string; default_auto_send_invoice?: boolean | null; default_payment_reminder?: boolean | null;
...
export default function Invoice() {
    ...
    useEffect(() => {
        if (!userId) return;
        const checkCompanyInfo = async () => {
            const { data, error } = await supabase.from("sellers_record").select("id, name, address, state, gstin, contact, email, logo_url, bank_name, account_no, ifsc_code, default_auto_send_invoice, default_payment_reminder, default_template").eq("user_id", userId).single();
