...
import html2canvas from 'html2canvas';
import { QRCodeCanvas } from "qrcode.react";
import { Download, Edit, Link as LinkIcon, Loader2, Phone, Mail, MapPin, Banknote, Landmark } from "lucide-react"; 
import { supabase } from "@/lib/supabase";
import { DashboardLayout } from "@/components/dashboard-layout";
...
export default function SellerCard() {
  ...
  const handleDownloadPDF = async () => {
    ...
    // Define a standard card size for PDF consistency (300 DPI equivalent)
    const cardWidthPx = 500; 
    const cardHeightPx = cardWidthPx / 1.66; 

    // Capture the front side
    const frontCanvas = await html2canvas(cardFrontRef.current, { scale: 3, backgroundColor: '#1C1C1E', width: cardWidthPx, height: cardHeightPx });
    const frontImgData = frontCanvas.toDataURL("image/jpeg", 1.0); 

    // Capture the back side
    const backCanvas = await html2canvas(cardBackRef.current, { scale: 3, backgroundColor: '#1C1C1E', width: cardWidthPx, height: cardHeightPx });
    const backImgData = backCanvas.toDataURL("image/jpeg", 1.0);
    
    // Initialize jsPDF (Use millimeters for standard print/export size)
  ...
  // Reusable card content for front and back - IMPROVED STYLING
  const CardFrontContent = () => (
    <div className="relative inset-0 rounded-2xl bg-[#1C1C1E] text-white shadow-2xl p-6 sm:p-8 flex flex-col justify-between overflow-hidden border border-gray-700/50">
      
      {/* Subtle Background Pattern */}
      <div className="absolute inset-0 bg-[radial-gradient(#ffffff22_1px,transparent_1px)] [background-size:16px_16px] [mask-image:radial-gradient(ellipse_50%_50%_at_50%_50%,#000_60%,transparent_100%)] opacity-20"></div>
      
      {/* Top Section: Logo & Name */}
      <div className="relative z-10">
        <div className="flex items-center gap-4">
          {seller.logo_url && (
            <img src={seller.logo_url} alt="Logo" className="w-16 h-16 sm:w-20 sm:h-20 object-contain rounded-xl bg-white/10 p-2 shadow-inner" />
          )}
          <div>
            <h2 className="text-2xl sm:text-3xl md:text-2xl font-extrabold tracking-tight text-indigo-400">{seller.name}</h2>
            {seller.title && <p className="text-gray-300 font-medium text-sm sm:text-base mt-1">{seller.title}</p>}
          </div>
        </div>
        {seller.description && <p className="text-xs sm:text-sm text-gray-400 mt-4 max-w-sm border-l-2 border-indigo-500 pl-3 italic">"{seller.description}"</p>}
      </div>

      {/* Bottom Section: Contact Info */}
      <div className="relative z-10 space-y-2 text-sm sm:text-base mt-6">
        {seller.contact && <p className="flex items-center gap-3 font-semibold"><Phone size={16} className="text-indigo-400 flex-shrink-0"/><span className="hover:text-indigo-300 transition-colors">{seller.contact}</span></p>}
        {seller.email && <p className="flex items-center gap-3"><Mail size={16} className="text-gray-400 flex-shrink-0"/><span className="text-gray-300 break-all">{seller.email}</span></p>}
        {seller.address && <p className="flex items-center gap-3 text-xs sm:text-sm text-gray-400"><MapPin size={16} className="text-gray-500 flex-shrink-0"/><span>{seller.address}</span></p>}
      </div>
      
      {/* Footer Branding */}
      <a href="https://vyapari.vercel.app/" target="_blank" rel="noopener noreferrer" className="absolute bottom-4 right-6 text-xs text-gray-500 font-semibold hover:text-indigo-400 z-20 transition-colors">
          Powered by <strong>Vyapari AI</strong>
      </a>
    </div>
  );

  const CardBackContent = () => {
    // ⚠️ CORRECTED: Helper function defined INSIDE the component function body
    const DetailRow = ({ label, value, icon: Icon }: { label: string, value?: string, icon: React.ElementType }) => (
      value ? (
        <div className="flex flex-col gap-0.5">
            <span className="text-gray-400 flex items-center gap-2 font-medium"><Icon size={14} className="text-gray-500"/>{label}</span>
            <span className="font-mono text-gray-200 text-sm pl-6 break-all">{value}</span>
        </div>
      ) : null
    );

    return (
      <div className="relative inset-0 rounded-2xl bg-[#1C1C1E] text-white shadow-2xl p-6 sm:p-8 flex items-center justify-between overflow-hidden border border-gray-700/50">
        {/* Background Pattern */}
        <div className="absolute inset-0 bg-[url('data:image/svg+xml,%3Csvg%20width%3D%2260%22%20height%3D%2260%22%20viewBox%3D%220%200%2060%2060%22%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%3E%3Cg%20fill%3D%22none%22%20fill-rule%3D%22evenodd%22%3E%3Cg%20fill%3D%22%23ffffff%22%20fill-opacity%3D%220.06%22%3E%3Cpath%20d%3D%22M36%2034v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6%2034v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6%204V0H4v4H0v2h4v4h2V6h4V4H6z%22/%3E%3C/g%3E%3C/g%3E%3C/svg%3E')] opacity-70"></div>
        
        {/* Official/Bank Info Section */}
        <div className="relative z-10 w-1/2 space-y-3 text-xs sm:text-sm font-sans">
          <h3 className="text-lg font-bold text-indigo-400 flex items-center gap-2"><Landmark size={18}/> Official Details</h3>
          
          <DetailRow label="GSTIN" value={seller.gstin} icon={Banknote} />
          <DetailRow label="PAN Number" value={seller.pan_no} icon={Banknote} />
          
          {seller.bank_name && (
            <div className="pt-2 border-t border-gray-700/70">
              <h4 className="text-sm font-bold text-gray-300 mb-1">Bank Details:</h4>
              <div className="flex flex-col gap-1">
                  <span className="text-gray-400 text-xs">{seller.bank_name}</span>
                  <span className="font-mono text-gray-200 text-sm">A/c: {seller.account_no}</span>
              </div>
            </div>
          )}
        ...
        {/* QR Code Section */}
        <div className="relative z-10 flex flex-col items-center flex-shrink-0 ml-4">
          <div className="p-2 sm:p-3 bg-white rounded-xl shadow-lg">
            <QRCodeCanvas value={cardBackQrUrl} size={130} fgColor={"#1A1A1A"}/>
          </div>
          <span className="mt-2 text-xs text-gray-400 text-center font-semibold">{seller.website ? "Visit Website" : "View Digital Card"}</span>
        </div>
      </div>
    );
  };
  ...
  return (
    <DashboardLayout>
      <div className="min-h-screen bg-gray-100 dark:bg-gray-950 flex flex-col items-center justify-center p-4 gap-10 lg:flex-row lg:items-start font-sans">
        
        {/* Column 1: The Interactive Card & Actions */}
        <div className="flex flex-col items-center w-full max-w-lg">
          <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-4 hidden sm:block">Digital Business Card</h1>
          
          {/* 3D Flip Card Container */}
          <div 
            ref={cardContainerRef}
            className="w-full max-w-[500px] aspect-[1.66] perspective-1200 cursor-pointer group rounded-2xl shadow-2xl relative"
            onClick={() => setIsFlipped(!isFlipped)}
          >
            {/* Elegant outer border/glow effect */}
            <div className="absolute inset-0.5 rounded-2xl bg-gradient-to-br from-indigo-500/50 to-gray-800/50 p-px pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-0"></div>
          ...
          {/* Action Buttons Below Card */}
          <div className="flex flex-wrap items-center justify-center gap-3 mt-8 w-full max-w-[500px]">
            <button onClick={() => router.push(`/${seller.user_id}/edit`)} className="flex items-center flex-1 gap-2 px-4 py-3 rounded-xl bg-indigo-600 text-white font-semibold hover:bg-indigo-700 shadow-lg shadow-indigo-500/50 transition-colors text-sm sm:text-base min-w-[120px] justify-center">
              <Edit size={18} /> Edit Card
            </button>
            <button onClick={handleDownloadPDF} className="flex items-center flex-1 gap-2 px-4 py-3 rounded-xl bg-white dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-gray-200 shadow border dark:border-gray-600 font-semibold hover:bg-gray-200 transition-colors text-sm sm:text-base min-w-[120px] justify-center">
              <Download size={18} /> Download PDF
            </button>
            <button onClick={handleCopyLink} className="flex items-center flex-1 gap-2 px-4 py-3 rounded-xl bg-white dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-gray-200 shadow border dark:border-gray-600 font-semibold hover:bg-gray-200 transition-colors text-sm sm:text-base min-w-[120px] justify-center">
        ...
        {/* Column 2: The Separate Share Panel */}
        <div className="w-full sm:w-72 flex-shrink-0 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-xl border dark:border-gray-700 flex flex-col items-center mt-6 lg:mt-0">
          <h3 className="text-xl font-bold text-center text-gray-800 dark:text-gray-200">Instant Sharing</h3>
          <p className="text-xs text-center text-gray-500 dark:text-gray-400 mt-1">Scan to instantly share your complete digital profile.</p>
          <div className="p-3 bg-white rounded-xl mt-5 shadow-inner">
            <QRCodeCanvas id="profile-qr-canvas" value={profileUrl} size={180} />
          </div>
          <button onClick={downloadProfileQR} className="w-full mt-6 flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-gray-800 text-white font-semibold hover:bg-black dark:bg-indigo-600 dark:hover:bg-indigo-700 transition-colors text-sm sm:text-base">
